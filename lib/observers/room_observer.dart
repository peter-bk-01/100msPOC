...
class RoomObserver implements HMSUpdateListener, HMSActionResultListener {
  ...
  Future<void> getToken() async {
    var res = await roomOverviewBloc.hmsSdk.getAuthTokenByRoomCode(
        roomCode: roomOverviewBloc.roomCode ?? "",
        userId: "${DateTime.now().millisecondsSinceEpoch}");
    log(res);
    HMSConfig config =
        HMSConfig(authToken: res, userName: roomOverviewBloc.name);
    roomOverviewBloc.hmsSdk.join(config: config);
  }
  ...
  void onMessage({required HMSMessage message}) {
    ...
    roomOverviewBloc
        .add(RoomOverviewRoomOverviewOnMessageReceived(message: message));
  }
  ...
  void onPeerUpdate({required HMSPeer peer, required HMSPeerUpdate update}) {
    ...
    if (update == HMSPeerUpdate.peerJoined) {
      ...
      roomOverviewBloc.add(RoomOverviewOnPeerJoin(hmsPeer: peer));
      roomOverviewBloc.add(RoomOverViewOnPeerUpdate(peer: peer));
    }
  }
  ...
  void onTrackUpdate(
    ...
    if (track.kind == HMSTrackKind.kHMSTrackKindVideo) {
      if (trackUpdate == HMSTrackUpdate.trackRemoved) {
        ...
        peer.networkQuality;
        roomOverviewBloc.add(RoomOverviewOnPeerJoin(
          hmsPeer: peer,
    ...
    if (track.kind == HMSTrackKind.kHMSTrackKindAudio) {
      if (trackUpdate == HMSTrackUpdate.trackRemoved) {
        ...
        roomOverviewBloc.add(RoomOverviewOnPeerJoin(
          hmsPeer: peer,
          hmsAudioTrack: track as HMSAudioTrack,
        ));
      }
    }
  }
  ...
  void onSuccess(
    ...
    if (methodType == HMSActionResultListenerMethod.sendBroadcastMessage) {
      var x = HMSMessage.fromMap(arguments!);
      roomOverviewBloc
          .add(RoomOverviewRoomOverviewOnMessageReceived(message: x));
    }
  }
